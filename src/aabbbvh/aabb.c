...
/*   Updated: 2024/08/20 22:20:50 by ael-mank         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int	check_axis(double min, double max, double origin,
		double direction, double *t_min, double *t_max)
{
	double	invD;
	double	t0;
	double	t1;
...
int	hit_aabb(t_ray r, t_aabb box, t_interval ray_t)
{
	...
	t_min = ray_t.min;
	t_max = ray_t.max;
	if (!check_axis(box.x.min, box.x.max, r.org.x, r.dir.x, &t_min, &t_max))
		return (0);
	if (!check_axis(box.y.min, box.y.max, r.org.y, r.dir.y, &t_min, &t_max))
		return (0);
	if (!check_axis(box.z.min, box.z.max, r.org.z, r.dir.z, &t_min, &t_max))
		return (0);
	return (1);
}
