...
/*   Updated: 2024/09/25 12:18:51 by ael-mank         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

t_quad	*create_quad(t_point3 start, t_vec3 u, t_vec3 v, t_material_type type,
		t_vec3 color)
{
	t_quad		*quad;
	t_material	*mat;
...
t_object	*add_quad(t_object *head, t_quad *quad)
{
	...
	bbox_diag1 = aabb_points(quad->start, vector_add(vector_add(quad->start,
					quad->u), quad->v));
	bbox_diag2 = aabb_points(vector_add(quad->start, quad->u),
			vector_add(quad->start, quad->v));
