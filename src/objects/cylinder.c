...
/*   Updated: 2024/09/17 13:46:38 by ael-mank         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_cylinder_uv(t_vec3 p, t_cylinder cylinder, double *u, double *v)
{
    ...
    *u = (theta + M_PI) / (2 * M_PI);
    *v = dot(relative_p, cylinder.axis) / cylinder.height + 0.5; // Normalize v to [0, 1]
}

static inline double	hit_cylinder(t_ray r, t_cylinder cylinder, t_interval ray_t, t_hitrecord *rec)
{
    t_vec3	oc;
    double	a;
    double	half_b;
    ...
    d = vector_subtract(r.dir, vector_scale(cylinder.axis, dot(r.dir, cylinder.axis)));
    oc = vector_subtract(r.org, cylinder.center);
    oc_proj = vector_subtract(oc, vector_scale(cylinder.axis, dot(oc, cylinder.axis)));
    ...
    hit_height = dot(vector_subtract(hit_point, cylinder.center), cylinder.axis);
    ...
    rec->t = root;
    rec->p = hit_point;
    outward_normal = vector_normalize(vector_subtract(vector_subtract(rec->p, cylinder.center), vector_scale(cylinder.axis, hit_height)));
